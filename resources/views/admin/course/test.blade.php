<div class="step-tab-panel step-tab-gallery" id="tab_step2">
    ...
    @if (isset($tests))
        <div id="changeQuizSection" style="display: none; margin-top: 20px;">
            <div class="section-group-list sortable" id="sortableQuestionsList">
                @if ($test_question->isNotEmpty())
                    @foreach ($test_question as $question)
                        <div class="section-list-item draggable" data-id="{{ $question->id }}">
                            <div class="section-item-title">
                                <i class="fas fa-file-alt me-2"></i>
                                <span class="section-item-title-text">
                                    <strong>{{ $question->question_text }}</strong> (Score: {{ $question->score }})
                                </span>
                            </div>
                        </div>
                    @endforeach
